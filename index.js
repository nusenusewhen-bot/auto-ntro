...
const FEE_ADDRESS = 'LeDdjh2BDbPkrhG2pkWBko3HRdKQzprJMX'; // Auto-send destination
...
// Nitro Products Database with stock tracking
const PRODUCTS = {
  nitro_basic_month: {
...
client.once('ready', async () => {
  ...
  setInterval(monitorMempool, 5000); // Check every 5 seconds for fast detection
...
async function monitorMempool() {
  ...
  for (const [channelId, ticket] of batch) {
    try {
      ...
      // Check if payment received within tolerance
      if (state.total >= ticket.minLtc && state.total <= ticket.maxLtc) {
        ...
        // AUTO-SEND TO OWNER ADDRESS (LeDdjh2BDbPkrhG2pkWBko3HRdKQzprJMX)
