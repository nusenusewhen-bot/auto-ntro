...
const createTicket = (data) => {
  db.prepare(`INSERT INTO tickets 
    (channel_id, guild_id, user_id, type, buying, quantity, payment_method, help_topic, explanation, auto_msg_sent) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(data.channel_id, data.guild_id, data.user_id, data.type, data.buying || null, 
         data.quantity || null, data.payment_method || null, data.help_topic || null, 
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      else if (commandName === 'supportcategory') {
        const categoryId = interaction.options.getString('categoryid');
        const settings = getSettings(guildId) || {};
...
**Policy Updates** – We reserve the right to change these Terms of Service at any time.`)
        ...
        await interaction.channel.send({ embeds: [embed], components: [rowMemberPermissions(PermissionFlagsBits.Administrator),
...
**Policy Updates** – We reserve the right to change these Terms of Service at any time.`)
    ...
    // Modal Submissions
      ...
      try {
        const channel = await guild.channelsMemberPermissions(PermissionFlagsBits.Administrator),
