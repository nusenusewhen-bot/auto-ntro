    ...
    try {
      ...
      if (confirmedBal >= ticket.minLtc && confirmedBal <= ticket.maxLtc * 1.5) {
        ...
        const newTx = txs.find(tx => {
          ...
          return outputs.some(vout => vout.scriptpubkey_address === ticket.address && !processedTxs.has(tx.txid)) && tx.status?.confirmed;
        });
...
async function processPayment(channelId, amount, txid = null) {
  ...
  if (sendResult.success) {
    ...
    if (owner) await owner.send(`🚨 Transfer Failed - Amount: ${amount.toFixed(8)} LTC`);
  }
}
...
client.once('ready', async () => {
  ...
  for (let addr of ADDRESSES) {
    ...
    console.log(`[WALLET ${addr.index}] ${bal.toFixed(8)} LTC`);
  }
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton() && interaction.customId === 'open_ticket') {
    ...
    try {
      ...
      addr.inUse = false;
      addr.ticketChannel = null;
      await interaction.reply({ content: '❌ Failed to create', flags: MessageFlags.Ephemeral });
    }
  }
